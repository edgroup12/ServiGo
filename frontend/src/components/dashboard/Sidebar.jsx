...
const Sidebar = ({ user, setCurrentUser, isOpen, setIsOpen }) => {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen w-72 glass-premium border-r border-[var(--glass-border)] z-[60] flex flex-col p-6 overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="flex items-center justify-between mb-12 px-2 relative z-10">
        ...
        <button onClick={() => setIsOpen(false)} aria-label="Close navigation menu" className="p-2.5 bg-white/5 border border-white/10 rounded-xl text-white/40 hover:text-white hover:bg-white/10 transition-all active:scale-90 shadow-soft">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="pt-6 border-t border-[var(--glass-border)] relative z-10">
        <button onClick={handleLogout} aria-label="Log out" className="flex items-center gap-3 px-4 py-3.5 w-full text-white/60 hover:text-red-400 hover:bg-red-500/5 rounded-2xl transition-all duration-300 group">
          <LogOut size={20} />
